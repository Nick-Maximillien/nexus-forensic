...
class ForensicRAG:
    ...
    Upgraded to Logic Fusion: Clinical Facts + Certification Stamp.
    """
    ...
    def retrieve_applicable_rules(
        ...
        # 3. MULTI-HEAD RETRIEVAL (Logic Fusion)
        # Partition the base_qs into two heads to ensure context diversity.
        ...
        # 4. EXECUTE FUSION (Force a 60/40 Split to prevent Clinical Dominance)
        head_a_limit = 6
        head_b_limit = 4
        
        clinical_results = list(get_scored_results(clinical_qs, head_a_limit))
        cert_results = list(get_scored_results(cert_qs, head_b_limit))

        # 5. COMBINE AND RETURN
        # Merges clinical facts with the certification scoring rules.
        return clinical_results + cert_results